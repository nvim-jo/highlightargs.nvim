local M = {}
local parser = require "highlightargs.parse"
local config = require "highlightargs.config"
local util = require "highlightargs.util"
local bufdata = require "highlightargs.bufdata"
local paint = require "highlightargs.paint"
local async = require "highlightargs.async"
...
local function find_and_paint_iteration(bufnr, task, co)
  ...
  vim.defer_fn(function()
    if coroutine.status(co) ~= "dead" and not task.stop and vim.api.nvim_buf_is_loaded(bufnr) then
      ...
      if running then
        if config.opts.highlight_arg_declarations then paint_nodes(bufnr, task.ns, arg_nodes) end
...
function M.enable()
  if not enabled then
    enabled = true
    local augroup = vim.api.nvim_create_augroup("Highlightargs", { clear = true })
...
function M.disable()
  if enabled then
    enabled = false
    vim.api.nvim_clear_autocmds { group = "Highlightargs" }
