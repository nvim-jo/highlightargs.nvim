...
local defaults = {
  ...
  highlight_arg_declarations = true,
  ...
  local function create_hl_groups()
    if M.opts.use_colorpalette then
      ...
      if vim.tbl_isempty(M.opts.highlight) then
        vim.api.nvim_set_hl(0, "Highlightargs", { fg = M.opts.color, default = true })
      else
        M.opts.highlight.default = true
        vim.api.nvim_set_hl(0, "Highlightargs", M.opts.highlight)
      end
    end

    if M.opts.extras.named_parameters then
      vim.api.nvim_set_hl(0, "@HighlightargsNamedParams", { link = "Highlightargs" })
    end
  end

  create_hl_groups()

  local augroup = vim.api.nvim_create_augroup("highlightargs-create-hlgroups", { clear = true })
